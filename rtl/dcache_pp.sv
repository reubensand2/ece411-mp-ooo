...
(
    ...
    logic primed;
    logic raw_hazard;
    assign ufp_raw_hazard = raw_hazard;
    ...
    // assign tag_index  = (stall || !cache_reg_next.valid) ? cache_reg.ufp_addr[8:5] : ufp_addr[8:5];
    assign tag_index  = (stall) ? cache_reg.ufp_addr[8:5] : ufp_addr[8:5];

    // assign data_index = tag_index;  // always same set
    assign data_index   = write_hit ? cache_reg.ufp_addr[8:5] : tag_index;
    ...
    /*      dirty array      */ 
    logic           dirty_select_fetch;
    logic [3:0]     dirty_write_enable_fetch;
    logic [3:0]     dirty_index_fetch;
    logic           dirty_din_fetch;
    logic [3:0]     dirty_dout_fetch;

    logic           dirty_select_comp;
    logic [3:0]     dirty_write_enable_comp;
    logic [3:0]     dirty_index_comp;
    logic           dirty_din_comp;
    logic [3:0]     dirty_dout_comp;
    logic [3:0]     dirty_corrected;

    assign dirty_index_comp = cache_reg.ufp_addr[8:5];      // addr from comp stage
    assign dirty_index_fetch = ufp_addr[8:5];               // addr from fetch stage
    ...
    assign dirty_corrected = ((state == s_compare) & ~primed) ? dirty_dout_fetch : dirty_dout_comp;
   ...
   // A read is required on a new unstalled request, when priming the pipeline,
    // OR when we detect a dirty miss and need to fetch the victim line for writeback!
    assign read_req = ((ufp_rmask != '0 || ufp_wmask != '0) && !stall) || 
                      (state == s_prime) || 
                      (state == s_compare && state_next == s_wb);

    always_comb begin
        for (integer unsigned j = 0; j < 4; j++) begin
            // csb is ACTIVE LOW (0). 
            data_csb[j] = ~(read_req || (data_write_enable[j] == 1'b0));
            tag_csb[j]  = ~(read_req || (tag_write_enable[j]  == 1'b0));
        end
    end

    // --- GLITCH-FREE CLOCK GATING (ICG LATCHES) ---
    logic [3:0] data_en_latched;
    logic [3:0] tag_en_latched;

    // Latch the active-high enable signals on the negative edge of the clock
    always_latch begin
        if (!clk) begin
            data_en_latched = ~data_csb;
            tag_en_latched  = ~tag_csb;
        end
    end
    ...
    generate for (genvar i = 0; i < 4; i++) begin : arrays
        mp_cache_data_array data_array (
            // Safely AND the latched enable with the clock
            .clk0       (clk && data_en_latched[i]),
            // Pass the inverted latched enable to the SRAM's active-low CSB port
            .csb0       (~data_en_latched[i]),       
        ...
        mp_cache_tag_array tag_array (
            .clk0       (clk && tag_en_latched[i]),
            .csb0       (~tag_en_latched[i]),               
        ...
        dp_ff_array dirty_array (
            ...
            .csb0       (dirty_select_comp),
            .web0       (dirty_write_enable_comp[i]),
            .addr0      (dirty_index_comp),
            .din0       (dirty_din_comp),
            .dout0      (dirty_dout_comp[i]),
            
            .csb1       (dirty_select_fetch),
            .web1       (dirty_write_enable_fetch[i]),
            .addr1      (dirty_index_fetch),
            .din1       (dirty_din_fetch),
            .dout1      (dirty_dout_fetch[i])
        );
    end endgenerate
    ...
    always_comb begin
        ...
        // dirty init
        dirty_select_comp   = '1;
        dirty_select_fetch  = '1;
        dirty_write_enable_comp = '1;
        dirty_write_enable_fetch = '1;
        dirty_din_comp  = 'x;
        dirty_din_fetch = 'x;
        ...
        cache_reg_next.ufp_wmask = ufp_wmask;
        cache_reg_next.ufp_wdata = ufp_wdata;
        cache_reg_next.valid = '0;

        write_hit = '0;

        if ((ufp_rmask != '0 || ufp_wmask != '0) && !stall) begin
        ...
        unique case (state)
            ...
            */
            s_compare : begin
                if (cache_reg.valid | primed) begin
                    ...
                    hit_word = hit_cache_line[32*cache_reg.ufp_addr[4:2] +: 32];
                    ...
                    if(|way_hit) begin
                        ...
                        // if write hit, enable only the way that hit
                        if(|cache_reg.ufp_wmask) begin
                            ...
                            // write data to matching way
                            data_write_enable = ~way_hit;
                            data_din[8*cache_reg.ufp_addr[4:0] +: 32]  = cache_reg.ufp_wdata;
                            data_wmask[cache_reg.ufp_addr[4:0] +: 4]   = cache_reg.ufp_wmask;
                            
                            // mark matching way as dirty
                            dirty_select_comp = '0;
                            dirty_write_enable_comp = ~way_hit;
                            dirty_din_comp = '1;
                        end
                    ...
                    // miss
                        ...
                        // if valid and dirty, send to write back
                        if(dirty_corrected[lru_decode] && valid_corrected[lru_decode])
                            state_next = s_wb;
                        else
                            state_next = s_alloc;
                    end
            ...
            */
            s_wb : begin
                ...
                // {evicted tag, matching index, 256-bit aligned}
                dfp_addr = {tag_dout[evict_way], cache_reg.ufp_addr[8:5], 5'b0};
            ...
            */
            s_alloc : begin
                ...
                if (dfp_resp) begin
                    ...
                    dirty_write_enable_comp[evict_way] = '0;
                    dirty_din_comp = '0;
